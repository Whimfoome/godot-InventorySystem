...
func refresh_slot():
	...
	if stack_amount <= 0:
		item_struct = IItem.new()
		stack_amount = 0
		inventory_comp.inv_slotstruct[slot_index] = item_struct
...
func _on_Button_gui_input(event):
	if event is InputEventMouseButton:
		...
		if event.button_index == BUTTON_LEFT and not event.pressed:
			...
			var interacter_inventory_comp = inventory_comp.interacter.get_node("InventoryComponent")
			
			if inventory_comp == interacter_inventory_comp:
				item_struct.i_use(inventory_comp.interacter)
				if item_struct.i_consumable:
					inventory_comp.inv_slotstack[slot_index] -= 1
					refresh_slot()
			else:
				if interacter_inventory_comp.add_to_inventory(item_struct, stack_amount):
